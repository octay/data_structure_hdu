...
int Menu() {
    ...
    for (int i = 0; i < 55; i++) cout << "-";// '-' * 55
    cout << endl;

    cout << endl << "1. I-Initialization " << endl;
    cout << endl << "2. E-Encoding " << endl;
    cout << endl << "3. D-Decoding " << endl;
    cout << endl << "4. P-Print " << endl;
    cout << endl << "5. T-Tree printing " << endl;

    cout << endl << "0. Q-Quit " << endl;
    for (int i = 0; i < 55; i++) cout << "-";// '-' * 55
    cout << endl;
...
int Init_HT(HuffmanTree &ht) {
    ...
    for (int i = 0; i < n; ++i) {       // read from console
        ...
        if (((i + 1) % 10 == 1) && ((i + 1) != 11)) cout << "st ";      // more intuitive scripts
        else if (((i + 1) % 10 == 2) && ((i + 1) != 12)) cout << "nd ";
        else if (((i + 1) % 10 == 3) && ((i + 1) != 13)) cout << "rd ";
        else cout << "th ";
...
int Select_min(HuffmanTree ht, int range, int &s1, int &s2) {   // i hate
    ...
    for (int i = 0; i < range; ++i) {
        if (ht.ht[i].parent == -1) {    // can be updated
            if (!s1 and ht.ht[s1].parent != -1) s1 = i;     // case: index 0 cannot be selected
            else if (!s2 and ht.ht[s2].parent != -1) s2 = i;
            else if (!s1 and !s2) ht.ht[i].weight < ht.ht[s1].weight ? s1 : s2 = i;
...
void Print_HT_raw(HuffmanTree &ht) {
    cout << endl << "index  ch  weight  left_child  right_child  parent" << endl;
    for (int i = 0; i < ht.ht_size; i++)
        cout << setw(6) << i << setw(6) << ht.ht[i].ch << setw(8) << ht.ht[i].weight << setw(9) << ht.ht[i].lChild
             << setw(8) << ht.ht[i].rChild << setw(6) << ht.ht[i].parent << endl;
}

void Print_HC_raw(HuffmanCoder &hc) {
    cout << endl << "ch  code" << endl;
    for (int i = 0; i < hc.hc_size; ++i) cout << setw(6) << hc.hc[i].ch << setw(6) << hc.hc[i].p_string << endl;
}
...
void Print_HT_weight(fstream &f, HuffmanTree &ht, int ind, int sh, int layer) {
    ...
    if (ht.ht[ind].rChild != -1) Print_HT_weight(f, ht, ht.ht[ind].rChild, 2, layer + 1);  // right center left
    switch (sh) {
        case 0:
    ...
    if (ht.ht[ind].lChild != -1) Print_HT_weight(f, ht, ht.ht[ind].lChild, 1, layer + 1);
...
void Print_HT_ch(fstream &f, HuffmanTree &ht, int ind, int sh, int layer) {
    ...
    if (ht.ht[ind].rChild != -1) Print_HT_ch(f, ht, ht.ht[ind].rChild, 2, layer + 1);  // right center left
    switch (sh) {
        case 0:
            if (ht.ht[ind].ch != '\0') {
    ...
    if (ht.ht[ind].lChild != -1) Print_HT_ch(f, ht, ht.ht[ind].lChild, 1, layer + 1);
    // this func is over with silence return
}
...
void Encoding(HuffmanCoder &hc, HuffmanTree &ht) {
    if (!ht.ht_size) Load_HT(ht);   // if func 1 has not been called
    Create_HC(hc, ht);
...
void Create_HC(HuffmanCoder &hc, HuffmanTree &ht) {
    ...
    for (int i = 0; i < hc.hc_size; ++i) {
        ...
        else if (((i + 1) % 10 == 3) && ((i + 1) != 13)) cout << "rd "; // this section wo3 ku1 si3
        else cout << "th ";
...
void Destroy_HC(HuffmanCoder &hc) {
    for (int i = 0; i < hc.hc_size; ++i) delete[] hc.hc[i].p_string;
    delete[] hc.hc;
}
...
void Decoding(HuffmanCoder &hc, HuffmanTree &ht) {
    ...
    cout << "read CodeFile.txt completed with code " << ch << endl << "the result is following ";
